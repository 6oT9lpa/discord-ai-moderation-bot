...
class OllamaClient:
    ...
    async def classify(
        ...
        """
        # Общие правила модерации, которые добавляются ко всем промптам классификации
        moderation_rules = """
ВАЖНЫЕ ПРАВИЛА МОДЕРАЦИИ (обязательно учитывай при классификации):

НЕ является нарушением (отвечай SAFE с confidence 0.0):
- Использование слов 'пидорас', 'еблан', 'хуй', 'блять' и т.д. в отношении СЕБЯ (например: 'я пидорас', 'я еблан' — это SAFE)
- Использование мата и оскорбительных слов в ВОПРОСАХ (например: 'что значит пидорас?' — это SAFE)
- Любая нецензурная лексика сама по себе (мат НЕ является нарушением)
- Легкие оскорбления (дурак, тупой, идиот и т.д.)

ЯВЛЯЕТСЯ нарушением (отвечай с confidence 1.0):
- Использование слов 'ДНР', 'ЛНР', 'Малороссия', 'Новороссия' в ЛЮБОМ контексте — это BULLYING
- Использование слова 'хохол', 'хохлы', 'хохлушка' и т.д. в ЛЮБОМ контексте — это BULLYING
- Заявления что Крым не является частью Украины — это BULLYING
- Оскорбления НАПРАВЛЕННЫЕ на другого человека ('ты пидорас', 'ты еблан') — это BULLYING
"""
...
ПОМНИ: Мат, нецензурная лексика и оскорбления — это НЕ спам. Спам — это реклама, массовые рассылки, приглашения на сторонние ресурсы.
...
ПОМНИ: Мат и нецензурная лексика сами по себе — это SAFE. Легкие оскорбления (дурак, тупой) — это SAFE. Слова типа 'пидорас', 'еблан' в отношении себя или в вопросах — это SAFE. Нарушением являются ТОЛЬКО: оскорбления направленные на другого человека, политические провокации (ДНР/ЛНР/хохол/Крым).
...
Сообщение: {text}"""
        ...
        prompt = prompt.format(text=text[:500], rules=moderation_rules)  # Ограничить длину
        
        system_prompt = "Ты автоматизированный API-парсер для модерации Discord сервера. Твоя единственная цель — выдавать структурированный JSON. Мат и нецензурная лексика — это НЕ нарушение. Оскорбления в свой адрес ('я пидорас') — это НЕ нарушение. Нарушением является ТОЛЬКО: спам/реклама, оскорбления НАПРАВЛЕННЫЕ на других, политические провокации (ДНР/ЛНР/хохол/Крым). ЗАПРЕЩЕНО писать любые рассуждения, теги <think> или извинения. В твоем ответе должен быть ТОЛЬКО текст JSON."
